...
var fetch = function(options) {
  ...
  //添加超时时间
  options.timeout = options.timeout||45000;
  ...
  options.error = function(error) {
    ...
    if (options.derror) {
      options.derror({
        errmsg: commLang.errorContent
      });
      return;
    }
    _errorFuncCache && _errorFuncCache({
      errmsg: commLang.errorContent
    });
  };
...
var util = {
  ...
  getRealUrl:function(url){
    if(window.hasLogin){
      url = config.serverBaseUrl+'interface'+url.replace(config.serverBaseUrl,'');
    }else{
      url = config.serverBaseUrl+'access'+url.replace(config.serverBaseUrl,'');
    }
    return url;
  },
};
