...
function fetch(opts, isInit){
    ...
    var options = {
        ...
        success: function(rs){
            ...
            if(opts.dataType==='text'){ // 默认dataType为json，非json一律当成普通文本，请自行处理
        ...
        fail: function(rs){
            ...
            if(rs.errMsg.indexOf('1032') > -1 && rs.errMsg.indexOf('应用会话超时') > -1){
                Alert.show({
                    ...
                    content:'会话超时，重新进入应用',
                    doOk: function(){
                        // appnest.worktable.closeWindow();
                        appnest.worktable.reloadApp();
                    }
                });
            }else{
                opts.error && opts.error({
                    errmsg: JSON.parse(rs.errMsg.trim()).faultstring
                });
            }
        }
    };
