...
module.exports = {
    async login(ctx){//登录校验用户合法性
        const rs = await accessModel.doLogin(ctx);
        if(rs){
            if(!rs.curlaborset){//用户不存在
                ctx.body = code.get(20001); 
            }else if(rs.curlaborset.labor.status != "ACTIVE"){//用户状态异常
                ctx.body = code.get(20002); 
            }else{
                ctx.body = {
                    ORGID:rs.curlaborset.labor.orgid,
                    maxSITEID:rs.curlaborset.labor.worksite
                }; 
            }
        }else{
