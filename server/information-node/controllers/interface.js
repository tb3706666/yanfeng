...
module.exports = {
    async searchs(ctx){
        const rs = await interfaceModel.searchs(ctx);
        if(rs){
            ctx.body = rs; 
        }else{
    ...
    async getuserkey(ctx){
        const rs = await interfaceModel.getuserkey(ctx);
        if(rs){
            ctx.body = rs; 
        }else{
            ctx.body = code.get(10000); 
    ...
    async uploaderServlet(ctx){
        const rs = await interfaceModel.uploaderServlet(ctx);
        if(rs){
            ctx.body = rs; 
        }else{
            ctx.body = code.get(10000); 
    ...
    async handleAttUpload(ctx){
        const rs = await interfaceModel.handleAttUpload(ctx);
        if(rs){
            ctx.body = rs; 
        }else{
            ctx.body = code.get(10000); 
    ...
    async save(ctx){
        const rs = await interfaceModel.save(ctx);
        if(rs){
            ctx.body = rs; 
        }else{
            ctx.body = code.get(10000); 
