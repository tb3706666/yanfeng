...
module.exports = {
    async ewyflist(ctx){
        ...
        let url_param = "method=data&categoryId=&nodeType=TEMPLATE&q.fdTemplate=143509bd3e98ced5329e24348c297245&orderby=fdId&ordertype=down&__seq=1525840077363&s_ajax=true&q.mydoc=approval";
        if(pageNum!=""){
            url_param = url_param + "&pageno=" + pageNum + "&rowsize=15&q.s_raq=0.5852299621033967";
        }
        url_param = url_param + "&orderby=docCreateTime&ordertype=down&__seq=1467621323685&s_ajax=true";
        let rs = await fetch(`${OA_URL}/ekp/fs/cwlc/fs_cwlc_main/cwlcMain.do?${url_param}`, {
            ctx: ctx,
            method: 'get',
            headers: {
    ...
    async ewyfquery(ctx){
        ...
        let url_param = "method=data&categoryId=&nodeType=TEMPLATE&q.fdTemplate=143509bd3e98ced5329e24348c297245&orderby=fdId&ordertype=down&__seq=1525841658608&s_ajax=true";
        if(param_subject!=''){
    ...
    async ewyfdetail(ctx){
        let rs = await fetch(`${OA_URL}/ekp/fs/cwlc/fs_cwlc_main/cwlcMain.do?method=view&fdId=${ctx.request.body.fdId}`, {
            ctx: ctx,
            method: 'get',
            headers: {
                "User-Agent": "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1) ; .NET CLR 2.0.50727; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; InfoPath.2)"
        ...
        rspObj.zt = $("form table[label='主表'] tbody").find('tr').eq(1).find('td').eq(1).find('div').eq(0).text().trim();
        rspObj.sqr = $("form table[label='主表'] tbody").find('tr').eq(2).find('td').eq(1).find('label').eq(0).text().trim();
        rspObj.sqrbm = $("form table[label='主表'] tbody").find('tr').eq(2).find('td').eq(5).find('div').eq(0).text().trim();
        
        rspObj.gsxx1 = $("form table[label='主表'] tbody").find('tr').eq(6).find('td').eq(1).find('div').eq(0).text().trim();
        rspObj.gsxx2 = $("form table[label='主表'] tbody").find('tr').eq(6).find('td').eq(3).find('div').eq(0).text().trim();
        rspObj.cbzx1 = $("form table[label='主表'] tbody").find('tr').eq(8).find('td').eq(1).find('div').eq(0).text().trim();
        rspObj.cbzx2 = $("form table[label='主表'] tbody").find('tr').eq(8).find('td').eq(3).find('div').eq(0).text().trim();
        rspObj.xmxx1 = $("form table[label='主表'] tbody").find('tr').eq(9).find('td').eq(1).find('div').eq(0).text().trim();
        rspObj.xmxx2 = $("form table[label='主表'] tbody").find('tr').eq(9).find('td').eq(3).find('label').eq(0).text().trim();
        rspObj.bzxx = $("form table[label='主表'] tbody").find('tr').eq(10).find('td').eq(1).find('label').eq(1).text().trim();

        let zrz = [];
        $("form table[label='主表'] tbody").find('tr').eq(10).find('td').eq(3).find("input[type='radio']").each((i,e)=>{
            zrz.push({
                text:$(e).parent("label").text().trim(),
                checked:$(e).attr("checked")?"true":"false"
            });
        });
        rspObj.zrz = zrz;    
        rspObj.fsyy = $("form table[label='主表'] tbody").find('tr').eq(13).find('td').eq(1).find('div').eq(0).text().trim();
        
        let mingxi = [];
        $("form table[label='主表'] tbody").find('tr').eq(15).find("table tr[type='templateRow'][style!='display:none']").each((i,e)=>{
            // console.log($(e))
            mingxi.push({
                xuhao:$(e).find("td").eq(0).text().trim(),
                td2:$(e).find("td").eq(1).find("div").eq(0).text().trim(),
                td3:$(e).find("td").eq(2).find("div").eq(0).text().trim(),
                td4:$(e).find("td").eq(3).find("div").eq(0).text().trim(),
                td5:$(e).find("td").eq(4).find("div").eq(0).text().trim(),
                td6:$(e).find("td").eq(5).find("div input").eq(0).attr("value"),
                td6s:$(e).find("td").eq(5).find("div").eq(0).text().trim(),
                td7:$(e).find("td").eq(6).find("div").eq(0).text().trim(),
                td8:$(e).find("td").eq(7).find("div").eq(0).text().trim(),
                td9:$(e).find("td").eq(8).text().trim(),
            });
        });
        rspObj.mingxi = mingxi;
        rspObj.zje = $("form table[label='主表'] tbody").find('tr').eq(16).find('td').eq(1).text().trim();
        rspObj.bzs = $("form table[label='主表'] tbody").find('tr').eq(17).find('td').eq(1).find('div').eq(0).text().trim();
