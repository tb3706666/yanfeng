...
module.exports = {
    ...
    async jiabansearch(ctx){
        ...
        let url = `${HR_URL}/flow/runtime/HistoryList.aspx?flowid=2511&pageindex=${ctx.request.body.start}&filters=[{"col":"a59","key":"${searchInfo}","op":"${ctx.request.body.sel}"},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""}]&sortColumn=`;
        // console.log(url)
        let rs = await fetch(url, {
            ctx: ctx,
            method: 'get',
        ...
        $('table[id="ctl03"] tr').filter(i=>i>0).each(function(index,e){
            ...
            let rs2 = $(e).find('td').eq(9).text().trim();
            let rs3 = $(e).find('td').eq(8).text().trim();
            let rs4 = $(e).attr('key');
            if(rs4.indexOf(".")>-1){
                rs4 = rs4.substring(0, rs4.indexOf("."));
            }
            tables.push({
                rs1:rs1,
                rs2:rs2,
    ...
    async faqi_jiabansearch(ctx){
        let url = `${HR_URL}/flow/runtime/InitList.aspx?flowid=2511&pageindex=${ctx.request.body.start}&sortColumn=`;
        // console.log(url)
        let rs = await fetch(url, {
            ctx: ctx,
            method: 'get',
    ...
    async jiabandetail(ctx){
        let url = `${HR_URL}/flow/runtime/ApproveIns.aspx?flowid=2511&taskid=${ctx.request.body.unid}`;
        ...
        let result = {
            ...
            zt:$("span[id = 'F1_1_C1']").text(),
            xm:$("span[id = 'F1_1_C3']").text(),
            bm:$("span[id = 'F1_1_C4']").text(),
            jbyy:$("xmp[id = 'F1_1_C8']").text(),
            jbkssj:$("span[id = 'F1_1_C10']").text(),
            jbjssj:$("span[id = 'F1_1_C11']").text(),
            jhjbsc:$("span[id = 'F1_1_C12']").text(),
            dysjjb:$("span[id = 'F1_1_C16']").text(),
            ljjbsc:$("span[id = 'F1_1_C17']").text(),
            jbdd:$("span[id = 'F1_1_C29']").text(),
        };
        ...
        result.butInfo = butArr;
         //是否申请人确认节点
        let F1_1_C13s = $("input[id='F1_1_C13']").attr('id');
        F1_1_C13s = F1_1_C13s?F1_1_C13s:'';
        result.ismansure = F1_1_C13s;
        if(F1_1_C13s!=''){
            result.F1_1_C13d = $("input[id='F1_1_C13']").val().split(" ")[0];
            result.F1_1_C13t = $("input[id='F1_1_C13']").val().split(" ")[1];
            result.F1_1_C14d = $("input[id='F1_1_C14']").val().split(" ")[0];;
            result.F1_1_C14t = $("input[id='F1_1_C14']").val().split(" ")[1];;
            result.F1_1_C15 = $("input[id='F1_1_C15']").text();
        }
    ...
    async jiabanyiban(ctx){
        let url = `${HR_URL}/flow/runtime/ViewIns.aspx?flowid=2511&action=edit&instanceid=${ctx.request.body.unid}&isinitiator=true`;
        let rs1 = await fetch(url, {
            ctx: ctx,
            method: 'get',
            headers: {
    ...
    async newjiaban(ctx){
        ...
        let {BADGE,PlanStartTime,PlanEndTime,OTREASON,FUJIANs,OTPLACE} = ctx.request.body;
        let rs = await fetch(url, {
            method: 'post',
            headers: {
            ...
            <soapenv:Header/>
            <soapenv:Body>
               <ns:LREGOT_REGISTER_DRAFT>
                <xml><![CDATA[<DataTable><RowData><BADGE>${BADGE}</BADGE><PlanStartTime>${PlanStartTime}</PlanStartTime><PlanEndTime>${PlanEndTime}</PlanEndTime><OTREASON>${OTREASON}</OTREASON><FUJIAN>${FUJIANs}</FUJIAN><OTPLACE>${OTPLACE}</OTPLACE></RowData></DataTable>]]></xml>
               </ns:LREGOT_REGISTER_DRAFT>
            </soapenv:Body>
         </soapenv:Envelope>`
        });
        ...
        console.log(rs.body.match(/<LREGOT_REGISTER_DRAFTResult xmlns="">(.*?)<\/LREGOT_REGISTER_DRAFTResult>/)[1])
        return rs.body.match(/<LREGOT_REGISTER_DRAFTResult xmlns="">(.*?)<\/LREGOT_REGISTER_DRAFTResult>/)[1];
    },   
};