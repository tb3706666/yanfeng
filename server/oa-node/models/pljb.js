...
module.exports = {
    async getDbList(ctx){
        ...
        return {
            dbList:tables,
            resit:$("script").eq(2).html().indexOf("请重新登录")>-1?'y':'n'
        }
    },
    async jiaqiansearch(ctx){
        let searchInfo = ctx.request.body.searchInfo;
        let url = `${HR_URL}/flow/runtime/HistoryList.aspx?flowid=2611&pageindex=${ctx.request.body.start}&filters=[{"col":"A4","key":"${searchInfo}","op":"${ctx.request.body.sel}"},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""},{"col":"A1","key":"","op":""}]&sortColumn=`;
        // console.log(url)
        let rs = await fetch(url, {
            ctx: ctx,
            method: 'get',
        ...
        $('table[id="ctl03"] tr').filter(i=>i>0).each(function(index,e){
            let rs1 = $(e).find('td').eq(1).text().trim();
            let rs2 = $(e).find('td').eq(4).text().trim();
            let rs3 = $(e).find('td').eq(2).text().trim();
            let rs4 = $(e).find('td').eq(4).text().trim();
            let rs5 = $(e).attr('key');
            if(rs5.indexOf(".")>-1){
    ...
    async jiaqiandetail(ctx){
        let url = `${HR_URL}/flow/runtime/ApproveIns.aspx?flowid=2611&taskid=${ctx.request.body.unid}`;
        ...
        let result = {
            ...
            bm:$("span[id = 'F1_5000_C7']").text(),
            sqyy:$("xmp[id = 'F1_5000_C8']").text(),
            jqsm:$("span[id = 'F1_1_C3']").text(),
            gjlx:$("span[id = 'F1_1_C11']").text(),
            dkcd:$("span[id = 'F1_5000_C17']").text(),
            sjdkcd:$("span[id = 'F1_5000_C18']").text(),

            qjcd:$("span[id = 'F1_1_C32']").text(),
            dw:$("span[id = 'F1_1_C21']").text(),
        };
        if($("input[name = 'F1_5000_C9']").length!=0){
            result.clfs = $("input[name = 'F1_5000_C9'][checked]").next('label').eq(0).text();
        }else{
            result.clfs = $("span[id = 'F1_5000_C9']").text();
        }
        if($("span[id = 'F1_5000_C1']").length==0){
            result.zt = $("input[id = 'F1_5000_C1']").val();
        }else{
            result.zt = $("span[id = 'F1_5000_C1']").text();
        }
        ...
        if($("span[id = 'F1_5000_C12']").length==0){
            result.kssj = $("input[id = 'F1_5000_C12']").val();
        }else{
            result.kssj = $("span[id = 'F1_5000_C12']").text();
        }
        if($("span[id = 'F1_5000_C16']").length==0){
            result.jssj = $("input[id = 'F1_5000_C16']").val();
        }else{
            result.jssj = $("span[id = 'F1_5000_C16']").text();
        }
        ...
        let jqlistArr = [];
        $("div[id='records_F2'] table tbody tr").each((i,e)=>{
            jqlistArr.push({
                td1:$(e).find('td').eq(0).find('span').eq(0).text(),
                td2:$(e).find('td').eq(1).find('span').eq(0).text(),
                td3:$(e).find('td').eq(2).find('span').eq(0).text(),
                td4:$(e).find('td').eq(3).find('span').eq(0).text(),
                td5:$(e).find('td').eq(4).find('span').eq(0).text(),
                td6:$(e).find('td').eq(5).find('span').eq(0).text(),
                td7:$(e).find('td').eq(6).find('span').eq(0).text(),
                td8:$(e).find('td').eq(7).find('span').eq(0).text(),
                td9:$(e).find('td').eq(8).find('span').eq(0).text(),
                td10:$(e).find('td').eq(9).find('span').eq(0).text()=='是'?'':$(e).find('td').eq(9).find('span').eq(0).text(),
                td11:$(e).find('td').eq(10).find('span').eq(0).text(),
            });
        })
        result.jqlistArr = jqlistArr;
    ...
    async jiaqianyiban(ctx){
        let url = `${HR_URL}/flow/runtime/ViewIns.aspx?flowid=2611&instanceid=${ctx.request.body.unid}`;
        let rs1 = await fetch(url, {
            ctx: ctx,
            method: 'get',
            headers: {
    ...
    async listDeal(ctx){
        let datas = Base64.decode(ctx.request.body.datas);
